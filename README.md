...
The layout.html is the base template that containts all the elements inside the head tag. It containts the footer and some jinja blocks:
   ...
   - [% block title %][% endblock %] for the title (replace the [% %] for curly braces)
    
   - [% if session["id"] %][% endif %] contains ]he navbar ]hat is show only if the user has log in (replace the [% %] for curly braces)
    
   -[% block main %][% endblock %] for all the content inside the main tag (replace the [% %] for curly braces)
    
   -[% block javascript %][% endblock %] for the script tag (replace the [% %] for curly braces)
